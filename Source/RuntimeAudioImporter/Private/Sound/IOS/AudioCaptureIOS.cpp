	...
	, uint32 NumFramesDesired)
{
	...
	// with error -66635. A second attempt after a short sleep avoids this issue.
	Status_Internal = AudioUnitInitialize(IOUnit);
	if (Status_Internal == -66635)
	{
		UE_LOG(LogRuntimeAudioImporter, Warning, TEXT("AudioUnitInitialize failed with error -66635, retrying after short delay"));
		
		// Sleep for a short time (5-10ms is typically sufficient)
		[NSThread sleepForTimeInterval:0.01];  // 10ms sleep
		...
		if (Status_Internal != noErr)
		{
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Unable to open capture stream due to error %d when initializing audio unit after retry"), static_cast<int32>(Status_Internal));
			bInitSucceeded = false;
			dispatch_semaphore_signal(semaphore); // Signal even on error
			return;
		}
	}
	else if (Status_Internal != noErr)
	{
